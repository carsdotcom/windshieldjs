...
describe("composer", function () {
    ...
    xit("should be called with at least two arguments", function (done) {
    ...
    xit("should be called with a context object and all additional arguments should be adapter functions", function (done) {
        var catcher = jasmine.createSpy();
    ...
    xit("should use component adapter if it exists and pass the page and context", function (done) {
    ...
    xit("if adapter does not exists, should use component Model if it exists", function (done) {
    ...
    xit("if neither adapter nor Model exists, use raw component data", function (done) {
        spyOn(internals, 'require').and.callFake(function (path) {
            if (path === '/mock/components/foo/adapter') throw new Error();
    ...
    xit("should resolve with a page object which has had associations for each adapter added to it", function (done) {
        composer.call(mocks.windshieldCallContext, mocks.genericContext, mocks.pageAdapter, mocks.assocAdapter1, mocks.assocAdapter2).then(function (page) {
    ...
    xit("should resolve with a page object which has had associations for each adapter added to it and the last association should take precedence in the case of a conflict", function (done) {
        composer.call(mocks.windshieldCallContext, mocks.genericContext, mocks.pageAdapter, mocks.assocAdapter1, mocks.assocAdapter2, mocks.assocAdapter3).then(function (page) {
