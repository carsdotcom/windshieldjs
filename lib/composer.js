...
function Composer(windshield) {
    ...
    function composer(context, adapters) {
        ...
        function resolver() {
            ...
            //
            //_.forIn(associations, function (associationComponents, name) {
            //
            //    var componentPromises = _.map(associationComponents, processComponent(context), null);
            //    var allComponentPromises = Promise.all(associationPromises).then(function (resolvedAssociation) {
            //        associations[name] = resolvedAssociation;
            //    });
            //
            //
            //
            ...
            return Promise.all(Object.keys(associations).map(function(assocName) {
                ...
                return Promise.all(associationPromises)
                    .then(function(components) {
                        associations[assocName] = components;
                    });
        ...
        return function (component) {
            ...
            if (adapter) {
                if (adapter.length <= 1) {
                    ...
                    // I'm removing deep clone since it was causing very MAJOR
