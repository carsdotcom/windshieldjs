...
function Composer(windshield, handlebars) {
    ...
    function composer(request) {
        ...
        function packagePage(assocData) {
            ...
            var pageFilter = request.route.settings.app.pageFilter;
            var page = {
                assoc: assoc,
                exported: exported,
        ...
        function runAssociationIterator() {
            return associationProcessorService.runAssociationIterator(context, request, components, context.associations, handlebars)
                .then(packagePage);
        }
