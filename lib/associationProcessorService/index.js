...
async function renderComponentSchema(definitionMap, renderComponent) {
    ...
    async function renderOneComponent(definitionGroupName, definition) {
        ...
        const nestedResults = await renderAssociationMap(childDefinitionMap);
    ...
    async function renderComponentFromArray(definitionGroupName, definitionGroup) {
        const renderedComponents = await Promise.map(definitionGroup, function (definintion) {
    ...
    async function renderAssociationMap(definitionMap) {
        ...
        return Promise.reduce(names, async function (accumulator, groupName) {
            const definitionGroup = definitionMap[groupName];

            if (!Array.isArray(definitionGroup)) {
                return accumulator;
            }

            const {markup, exported} = await renderComponentFromArray(groupName, definitionGroup);
