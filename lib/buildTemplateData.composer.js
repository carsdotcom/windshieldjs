...
function composer(componentMap) {
    ...
    async function buildTemplateData(request) {
        ...
        const updatedPageContext = await pageDefinitionService(request);
        const pageObject = await runAssociationIterator(updatedPageContext);
        logPageObject(pageObject);
        const filteredPageObject = await filterFn(pageObject);

        return parsePageDataForVision(filteredPageObject);
        ...
        function logPageObject(page) {
            if (process.env.WINDSHIELD_DEBUG) {
                request.server.log('info', JSON.stringify(page, null, 4));
            }
        }

        async function runAssociationIterator(updatedContext) {
            const {layout, attributes} = updatedContext;

            const renderer = componentMap.composeFactory(updatedContext, request);
            const {associations} = updatedContext;

            const renderedComponentCollection = await renderComponentSchema(associations, renderer);

            return packagePage(renderedComponentCollection, layout, attributes);
        }
    }
}
...
function packagePage(renderedComponentCollection, layout, attributes) {
    const exported = renderedComponentCollection.exportedData;
    return {
