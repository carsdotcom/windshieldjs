...
function Component(implementation, componentName) {
    ...
    component.render = function (definition, context, request, assocName, calledName) {
        ...
        if (implementation.cache) {
            let cacheConfig = cache.processConfig(implementation.cache, request);
            const cacheKey = cache.getKey(`component.${calledName}`, request, cacheConfig);
            return cache.getObject(cacheKey)
                .catch((err) => callThru().then((result) => cache.putObject(cacheKey, result, _.pick(cacheConfig, 'ttl')).then(() => result)));
        } else {
            return callThru();
        }
