...
function Renderer(windshield) {
    ...
    function renderer(reply) {
        function createTemplatePartial(name, component) {
            var templates = (component.templates || {});
            var template = name || 'default';
            component.component = component.component || 'componentNotFound';
            component.partial = 'partial_' + component.component + '_' + template;
            var filePromise;
            if (templates[template]) {
                filePromise = templates[template];
            } else if (templates.default) {
                filePromise = templates.default;
            } else {
                filePromise = Promise.reject(new ComponentNotFoundError('default template for ' + component.component + ' not defined.', component));
            }
        ...
        return function (data) {
            var componentPromises = Object.keys(data.associations)
                .map(function(name) {
                    ...
                    return _.map(component, _.partial(createTemplatePartial, name));
                })
