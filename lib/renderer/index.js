...
module.exports = function (reply) {
    ...
    return function (data) {
        ...
        return Promise.all(componentPromises).then(function (partials) {
            ...
            try {
                reply.view(path.join('layouts', 'default'), data);
                return;
            } catch (e) {}
            throw new Error("No layout template found when calling reply.view from renderer. Here's the data: " + JSON.stringify(data));
        });
    };
};
